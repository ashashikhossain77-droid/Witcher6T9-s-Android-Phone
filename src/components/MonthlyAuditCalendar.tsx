...
export const MonthlyAuditCalendar: React.FC<MonthlyAuditCalendarProps> = ({
  ...
  const monthlyStats = useMemo(() => {
    ...
    return {
      ...
      perfectDays,
      actionNeededDays,
      totalWorkingDays: calendarCells.filter(c => !c.isWeekend).length
    };
  }, [calendarCells]);
          ...
          <div className="p-3.5 rounded-xl bg-white border border-[#e7e1d5] shadow-2xs">
            ...
            <div className="font-display text-2xl font-bold text-[#17343a] font-mono-numbers">
              ...
              <span className="text-xs text-[#527078] font-normal font-sans">
                / {monthlyStats.totalWorkingDays} days
              </span>
            </div>
            <span className="text-[10px] text-[#527078]">
              {Math.round((monthlyStats.auditedDaysCount / (monthlyStats.totalWorkingDays || 1)) * 100)}% Coverage
            </span>
          </div>
      ...
      <div className="rounded-2xl border border-[#d9d2c2] bg-[#fbfaf6] p-5 sm:p-6 shadow-xs space-y-4">
        ...
        <div className="grid grid-cols-7 gap-1.5 sm:gap-2">
          ...
          {calendarCells.map(cell => {
            ...
            let cardBorder = isSelected ? 'border-[#176f78] ring-2 ring-[#176f78]/30 shadow-md' : 'border-[#e7e1d5] hover:border-[#176f78]/60';
            ...
            return (
              <button
                ...
                className={`min-h-[76px] sm:min-h-[92px] rounded-xl p-2 sm:p-2.5 text-left transition-all relative flex flex-col justify-between ${
                  isSelected ? 'bg-white' : cell.isWeekend ? 'bg-[#f1eee6]/50' : 'bg-white hover:bg-[#fbfaf6]'
                } border ${cardBorder}`}
              >
                ...
                <div className="flex items-center justify-between w-full">
                  ...
                  {cell.isWeekend ? (
                    <span className="text-[9px] font-semibold text-[#738287] uppercase">Off</span>
                  ) : cell.doneCount > 0 ? (
                    <span
                ...
                <div className="my-1">
                  <div className="h-1.5 w-full rounded-full bg-[#f1eee6] overflow-hidden">
                    <div
                      ...
                      style={{ width: `${cell.compliancePct}%` }}
                    />
                  </div>
                </div>
